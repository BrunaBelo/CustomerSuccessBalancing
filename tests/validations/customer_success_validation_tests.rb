...
class Validations::CustomerSuccessValidationTest < Minitest::Test
  def test_allowed_id
  ...
  def test_id_biggest_then_limit
    validator = described_class.new(
      costumer_success: { id: 200_000, score: 10 }
    )
    assert_equal false, validator.valid?
  end
  ...
  def test_available_costumer_success
    validator = described_class.new(
      costumer_success: { id: 10, score: 10 },
  ...
  def test_not_available_costumer_success
    validator = described_class.new(
      costumer_success: { id: 10, score: 10 },
